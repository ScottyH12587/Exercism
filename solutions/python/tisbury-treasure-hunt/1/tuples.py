...
def compare_records(azara_record, rui_record):
    ...
    """
    compare_values = convert_coordinate(azara_record[1])
    if compare_values == rui_record[1]:
        return True
    else:
        return False
...
def create_record(azara_record, rui_record):
    ...
    if compare_records(azara_record, rui_record):
        tuple_concatenated = azara_record + rui_record
        return tuple_concatenated
    else:
        return "not a match"
...
def clean_up(combined_record_group):
    ...
    for index in combined_record_group:
        new_tuple = (index[0], index[2], index[3], index[4])
        combined_string += f"{new_tuple}\n"
